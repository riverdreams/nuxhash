...
class ExcavatorServer(object):
    ...
    def _subscribe(self):
        ...
        self.send_command('subscribe', ['nhmp.'+region+'.nicehash.com:'+NHMP_PORT,
                                        wallet+'.'+worker+':x'])
    ...
    def start_work(self, algorithm, device, benchmarking=False):
        ...
        if benchmarking:
            response = self.send_command(
                    'worker.add', ['benchmark-'+algorithm, device_id])
        else:
...
class ESAlgorithm(ESResource):
    ...
    def _create(self):
        if self._benchmark:
            self._server.send_command(
                    'algorithm.add', ['benchmark-'+self._algorithm])
        else:
    ...
    def _destroy(self):
        if self._benchmark:
            self._server.send_command(
                    'algorithm.remove', ['benchmark-'+self._algorithm])
        else:
...
class ExcavatorAlgorithm(miner.Algorithm):
    ...
    def __init__(self, parent, excavator_algorithm, **kwargs):
        ...
        miner.Algorithm.__init__(
            self, parent, name='excavator_'+excavator_algorithm,
